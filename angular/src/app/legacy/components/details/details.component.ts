...
export class DetailsComponent implements OnInit {
	...
	getTenant(id: string | null): void {
		this.tenantsService.getItem(id).subscribe(
			(tenant: null) => {
	...
	setAvailableStatus(status: any): void {
		...
		this.tenantsService.update(this.currentTenant.id, data).subscribe(
			(response) => {
	...
	updateTenant(): void {
		this.tenantsService
			.update(this.currentTenant.tenantId, this.currentTenant)
			.subscribe(
				(response) => {
					console.log(response);
	...
	deleteTenant(): void {
		this.tenantsService.delete(this.currentTenant.tenantId).subscribe(
			(response) => {
				console.log(response);
